...
function init(){
    ...
    yBoite = 100;
    zBoite = 0.01;
    volumeBoite = xBoite* yBoite;
    volumeRestant = volumeBoite;

	camera.position.x = 20;
	camera.position.y = 80;
	camera.position.z = 600;
	camera.rotation.x = -0.049999999999999998;
	camera.rotation.y = -0.0300000000000000034;
	camera.rotation.z = -0.00046707480056708793;
...
function move() {
    ...
    /*q*/	case 65: camera.rotation.z += 0.005; break;
		/*d*/	case 69: camera.rotation.z -= 0.005; break;
...
function placerObj(bloc, point){
	...
	var geometry = new THREE.CubeGeometry( bloc.L, bloc.h, 0.01 );
    var material = new THREE.MeshBasicMaterial( { color: colorMat } );
	tmpObj = new THREE.Mesh( geometry, material );
	tmpObj.position = new THREE.Vector3(point.x+(bloc.L/2), point.y+(bloc.h/2), 0);
...
function ajouterListeObj(){
	...
	if((long!='')&&(haut!='')){
		tmpObj = new Bloc(0.01, long, haut);
...
function afficherListeObj(){
	var listeInnerHTML = '<tr><th>Id</th><th>Long</th><th>Haut</th><th>Coût</th></tr>';
	var i=0;
	listeObjaPlacer.forEach(function(Obj){
		listeInnerHTML+='<tr id="'+i+'" style="color:red"><td>'+i+'</td><td>'+Obj.L+'</td><td>'+Obj.h+'</td><td>' +Obj.volume/100+'</td></tr>';
		i+=1;
	});

  listeObjPlaces.forEach(function(Obj){
    listeInnerHTML+='<tr id="'+i+'" style="color:blue"><td>'+i+'</td><td>'+Obj.L+'</td><td>'+Obj.h+'</td><td>' +Obj.volume/100+'</td></tr>';
    i+=1;
  });
...
function genererListeObj(){
	...
	while(listeObjaPlacer.length > 0){
	    ...
	    if(res.length>0){
	  		...
	  		placerObj(new Bloc(0.01, Obj.L ,Obj.h), Pts);
	    }else{
...
function findBestSolution(){
	...
	while (!found){
		...
		logs += "\n-- test point "+i+" ("+cur_pts.x+","+cur_pts.y+")";
		...
		while (!found)
    {
			...
			logs += "\n------ test bloc "+j+" ("+cur_blc.L+","+cur_blc.h+")";

			cur_blc.rotate1();
			if(isMatching(cur_blc, cur_pts))
      {
	...
	//Mise à jour des points de placement avec le bloc trouvé
	var res = [];

	  if(found){
	  	listePointsPlacement.push(new Point(point.x+Number(bloc.L), point.y, 0));
		listePointsPlacement.push(new Point(point.x, point.y+Number(bloc.h), 0));
		listePointsPlacement.push(new Point(point.x+Number(bloc.L), point.y+Number(bloc.h), 0));
		...
		logs+="\nPlacement du bloc ("+bloc.L+","+bloc.h+") en ("+point.x+","+point.y+")\n";
		trierListeObj();
	...
	if(x_limit>xBoite){ logs += "\n---------- intersect xLimit("+xBoite+")"; return false; }
		...
		listeObjPlaces.forEach(function(Obj){
			if(Obj.intersect(bloc, point)){
				intersect = true;
				logs += "\n---------- intersect ("+Obj.L+","+Obj.h+") en ("+Obj.x+","+Obj.y+")";
			}
		});
...
function lireFichierListeObj(){
	...
	reader.onload = function(e) {
	    ...
	    objets.forEach(function(objet){
			...
			listeObjaPlacer.push(new Bloc(dims[0],dims[1],1));
		});
		afficherListeObj();
	};
